<?php
/*一队列池一个文件，这个是示范，改下面的队列池名称对应的地方就可以了，一个队列池要弄一个文件哦*/
...
while(true){
    $oid=$redis->lpop('orderoid0');
    $a=$DB->get_row("select kcname,status,process,remarks from qingka_wangke_order where oid='$oid'");
    if($oid!=''){
