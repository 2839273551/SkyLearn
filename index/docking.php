					...
					<div class="layui-tab-content">
						<div class="layui-tab-item layui-show">
							<blockquote class="layui-elem-quote">
								对接地址：<span
									style="color: #5FB878;">http://<? echo ($_SERVER['SERVER_NAME']); ?>/api.php?act=add</span>
							</blockquote>
							<blockquote class="layui-elem-quote">
								请求方式：POST
							</blockquote>
                    ...
                    <tr>
                      ...
                      <td>" . $rs['name'] . "</td>
						...
						<div class="layui-tab-item">
							...
							<pre lay-title="平台标识[PHP]" class="layui-code">//xm标识 放在/Checkorder/xdjk.php 文件，SkyLearn模板则无视
"xm" => "xm学习平台", </pre>
							<hr>
							<pre lay-title="查课代码[PHP]" class="layui-code">//xm学习平台查课接口 放在/Checkorder/ckjk.php 文件，SkyLearn模板则无视
if ($type == "xm") {
		...
		return $result;}</pre>
							<hr>
							<pre lay-title="下单代码[PHP]" class="layui-code">//xm学习平台下单接口 放在/Checkorder/xdjk.php 文件，SkyLearn模板则无视
	else if ($type == "xm") {
		$data = array("uid" => $a["user"], "key" => $a["pass"], "platform" => $noun, "school" => $school, "user" => $user, "pass" => $pass, "kcname" => $kcname, "kcid" => $kcid);
		$xm_rl = $a["url"];
		$xm_url = "$xm_rl/api.php?act=add";
		$result = get_url($xm_url, $data);
		$result = json_decode($result, true);
		if ($result["code"] == "0") {
		...
		return $b;} </pre>
							<hr>
							<pre lay-title="进度代码[PHP]" class="layui-code">//xm学习平台进度新接口 放在/Checkorder/jdjk.php 文件，SkyLearn模板则无视
 if ($type == "xm") {
	$xm_rl = $a["url"];
 ...
 <!--else if ($type == "xm") {
		...
		$xm_rl = $a["url"];
		$xm_url = "$xm_rl/api.php?act=chadan";
		$result = get_url($xm_url, $data);
		$result = json_decode($result, true);
		if ($result["code"] == "1") {
			foreach ($result["data"] as $res) {
		...
		return $b;}--></pre>
							<hr>
							<pre lay-title="补刷代码[PHP]" class="layui-code">//xm学习平台补刷 放在/Checkorder/bsjk.php 文件，SkyLearn模板则无视
	if ($type == "xm") {
		...
		return $result;}</pre>
							...
							<pre lay-title="暂停代码[PHP]" class="layui-code">//xm学习平台暂停 放在/Checkorder/ztjk.php 文件，SkyLearn模板则无视	    
	if ($type == "xm") {
$data = array("uid" => $a["user"], "key" => $a["pass"], "id" => $yid);
$xm_rl = $a["url"];
...
return $result;}
				...
				$b = array("code" => -1, "msg" => "接口异常，请联系管理员");
							...
							<hr>
							<pre lay-title="修改代码[PHP]" class="layui-code">//xm学习平台改密 放在/Checkorder/xgjk.php文件，SkyLearn模板则无视
			if ($type == "xm") {
...
return $result;}
				...
				$b = array("code" => -1, "msg" => "接口异常，请联系管理员");
							...
							<hr>
							<pre lay-title="秒刷代码[PHP]" class="layui-code">//xm学习平台秒刷 放在/Checkorder/msjk.php文件，SkyLearn模板则无视
			if ($type == "xm") {
$data = array("uid" => $a["user"], "key" => $a["pass"], "id" => $yid);
$xm_rl = $a["url"];
...
return $result;}
				...
				$b = array("code" => -1, "msg" => "接口异常，请联系管理员");
						...
						<div class="layui-tab-item">
							...
							<blockquote class="layui-elem-quote">请求头部：<span
									...
									style="color: #00aaaa">Content-Type: application/json; charset=UTF-8</span>
							</blockquote>
							<blockquote class="layui-elem-quote">对接UID：<span style="color:#f07178"
									onclick="copyToClip('<?php echo $userrow['uid']; ?>')"><?php echo $userrow['uid']; ?></span>
							</blockquote>
							<blockquote class="layui-elem-quote">对接KEY：<span style="color:#f07178"
	...
	"msg": "提示信息",//返回信息
						...
						<div class="layui-tab-item flex-container">
							<blockquote class="layui-elem-quote">
								SkyLearn一键对接【所有29对接SkyLearn】上架插件 <a href="../assets/download/SkyLearn对接插件.zip"
									class="layui-btn layui-btn-xs layui-btn-normal">点击下载&gt;&gt;</a>
							</blockquote>
							<blockquote class="layui-elem-quote">
								订单批量同步【所有29对接SkyLearn】 <a href="../assets/download/订单批量同步.zip"
									class="layui-btn layui-btn-xs layui-btn-normal">点击下载&gt;&gt;</a>
							</blockquote>
						</div>
						<div class="layui-tab-item">
