		        ...
		        <table class="table table-striped">
		          ...
		          <tbody>
		            <tr v-for="res in row.data">
		                <td><button class="btn btn-xs btn-primary" data-toggle="modal" data-target="#modal-update" @click="storeInfo=res">编辑</button>&nbsp;<button class="btn btn-xs btn-primary" @click="checkBalance(res.hid)">余额查询</button>&nbsp;
		                
		                <button class="btn btn-xs btn-primary"data-toggle="modal" @click="del(res.hid)">删除</button>&nbsp;
		                </td>
		            	<td>{{res.hid}}</td>
		            	<td>{{res.name}}</td>
		            	<td>{{res.pt}}</td>
	...
	methods:{
		get:function(page){
		  var load=layer.load(2);
			...
			var load=layer.load(2);
		 		this.$http.post("/apisub.php?act=huoyuan_del",{hid:hid},{emulateJSON:true}).then(function(data){	
          	layer.close(load);
          	if(data.data.code==1){	
          		  vm.get(1);		                     	
