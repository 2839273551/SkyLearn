...
?>
     <div class="app-content-body ">
	    ...
	    <div class="panel panel-default" >
		    <div class="panel-heading font-bold bg-white">密价列表&nbsp;<button class="btn btn-xs btn-success" data-toggle="modal" data-target="#modal-add">添加</button>
		      <div class="pull-right">
    <input type="text" v-model="filterUID" placeholder="输入UID筛选" class="form-control input-sm" style="display: inline-block; width: 200px;">
    <button class="btn btn-sm btn-primary" @click="filterByUID">筛选</button>
  </div>
		    </div>
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form class="form-horizontal" id="form-update">
                           ...
                           <div class="form-group">
    <label class="col-sm-3 control-label">筛选商品</label>
    <div class="col-sm-9">
        <input type="text" class="form-control" v-model="filterName" placeholder="输入商品名筛选...">
    </div>
</div>
                            ...
                            <div class="form-group">
                               ...
                               <select v-model="upm.cid" class="form-control">
	                              <option v-for="item in filteredClasses" :value="item.cid">{{ item.name }}</option>
	                            </select>
                               </div>
                            </div>
                            <div class="form-group">
...
vm=new Vue({
    ...
    filteredClasses() {
        if (this.filterName) {
            return this.classes.filter(item => item.name.toLowerCase().includes(this.filterName.toLowerCase()));
        }
        return this.classes;
    }
},
methods:{
		            filterByUID: function() {
    this.get(1); // 重新获取第一页数据
  },
		            getClasses:function(){
                var obj=this;
                axios.get("/apisub.php?act=getclass")
                    .then(function (response) {
                        obj.classes = response.data.data;
                    })
		  ...
		  var load=layer.load(2);
 			this.$http.post("/apisub.php?act=mijialist",{filterUID: this.filterUID},{emulateJSON:true}).then(function(data){	
	          	layer.close(load);
	          	if(data.data.code==1){			                     	
	          		this.row=data.body;			             			                     
	          	}else{
