...
- 已部署到 `https://sk.yunxnet.cn/index/`：
  ...
  - 2026-09-14 升级版本发布体系（Semantic Versioning `v8.x`）：
    ...
    - `v8.3.9` 进度条醒目度强力升级：重构为高饱和彩色药丸状态徽章 + 粗体等宽百分比 + 6px 鲜艳实心渐变轨道条，视觉焦点极其突出；
    - `v8.4.0` 调度中心智能归档防刷引擎：已完成/已取消/已退款或无补刷需求的100%订单全面智能归档，彻底杜绝进入下一轮同步轮询池，全网节流降负载；
    - `v8.5.0` 调度中心三大分频业务引擎重塑：彻底取缔旧版 8 个碎片字母任务，合体为“新订单自动出单(高频)”、“活跃看课同步(常规频)”、“待考试与平时分慢速巡检(慢频)”，建立补单单次受控机制杜绝循环轰炸；
    - `v8.5.1` 调度日志全自动滚动瘦身引擎：调度执行后自动清理超3天过期日志与孤立任务残留，单任务上限截断200条，支持一键清空日志，数据库永久保持轻盈；
    - `v8.5.2` 全网清退免费下单功能：彻底移除签到赠送免费次数、系统设置中免费课程配置，清除查课与下单界面全部免费额度标签，严格恢复标准按单计费体系。
